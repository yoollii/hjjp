...
//        	 alert( document.getElementById("key").value);
        	...
        	if($scope.selectedItem.properties[index].key=="oryx-usertaskassignment"){
        		...
        		/*if(pNode){
        			...
        			var nodeValue=$scope.selectedItem.properties[index].value.assignment.candidateGroups[0].value;
        			
        			$http({
		            			url: "loadForm/changeIds?nodeValue="+nodeValue,
		         				cache:false,
		         				async:false,
		         				method: "POST",
		         				data:{
		            		   ...
		            		   $scope.pop=false;
		            		   $scope.selectedItem.properties[index].value.assignment.candidateTrueUsers = data;
		            		   layer.close(index);
		            	   }).error(function(){
		            		   $scope.pop=false;
		            	   })
        	...
        	if($scope.selectedItem.properties[index].key=="oryx-fieldpower"){
        		var taskid=$scope.selectedItem.properties[0].value
        		var formkey= document.getElementById("key").value;
        		var proceModelId=document.getElementById("modelid").value;
        		if(""==taskid || taskid==null){
        			alert("未输入用户任务id");
        		}else if(formkey=="" || proceModelId==""){
        			alert("未获取到该有的值");
        		}
        		else{
        	                 layer.open({
        	 		               title : "用户任务授权",
        	 		               type: 2,
        	 		               content : "model/TaskpowerPageNew?processId="+proceModelId+
        	 		               "&tableCode="+formkey+"&taskKey="+taskid,
        	 		               area : [ '1000px', '600px' ],
        	 		               fixed : false, // 不固定
        	 		               maxmin : true,
        	 	                   btn : [ '确定', '取消' ],
        	 		               yes : function(index) {
        	 		            	  //console.log(jQuery("iframe").contents().find("#fieldmap").serialize());
        	 		            	   $http(
        	 		            		  {
        	 		            			 url: "model/changePower",
        	 		         				...
        	 		         				async:false,
        	 		         				method: "POST",
        	 		         				data:jQuery("iframe").contents().find("#fieldmap").serialize(),
        	...
        	if($scope.selectedItem.properties[index].key=="oryx-formkeydefinition"){
        		// oryx-multiinstance_type
        		if($scope.selectedItem.properties[index].value!=true){
        			$scope.selectedItem.properties[index].value=true;
        			// alert($scope.selectedItem.properties[index].value);
...
function GroupPower(proceModelId,formkey,taskid) {
	$http({
				url:"TaskpowerPage",
				cache:false,
				async:false,
				data:{
					processId:proceModelId,
					tableCode:formkey,
					taskKey:taskid
					}
	           }
